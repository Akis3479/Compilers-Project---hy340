...
term : LEFT_PARENTHESIS expr RIGHT_PARENTHESIS         {$$ = $2; printf("term <- ( expr ) :line  %d  \n", yylineno);}
     ...
     | PLUS_PLUS lvalue {
                                ...
                                if($2->type == tableitem_e){
                                        ...
                                        emitQuad(add, $2, $2, newexpr_constint(1), -1, yylineno);
                                        $$ = newexpr(arithexpr_e);
                                        $$->sym = newtemp(symtab);
     ...
     | lvalue   {
                ...
                PLUS_PLUS       {
                                        ...
                                        if($1->type == tableitem_e){
                                                ...
                                                emitQuad(assign, $$, val, NULL, -1, yylineno);
                                                emitQuad(add, val, val, newexpr_constint(1), -1,yylineno);
                                                emitQuad(tablesetelem, $1, $1->index, val, -1, yylineno);
                                        }else{
...
assignexpr : lvalue     {
                                ...
                                local=0;
                                                ...
                                                if($1->type == tableitem_e){
                                                        emitQuad(tablesetelem, $1, $1->index, $4, -1, yylineno);
...
member : lvalue DOT ID                                  {
                                                                ...
                                                                $$ = member_item(symtab, $1, yylval.id);
...
objectdef : LEFT_BRACKET elist RIGHT_BRACKET            {
                                                                ...
                                                                int i;
                                                                for(i=0; $2; $2 = $2->next){
                                                                        emitQuad(tablesetelem, t, newexpr_constint(i++), $2, -1, yylineno);
                                                                }
                                                                $$ = t;
...
indexed : indexedelem indexeds	                                {$$ = $1; $$->next = $2; printf("indexed <- indexedelem indexeds :line  %d  \n", yylineno);}
	;
	
indexeds : COMMA indexedelem indexeds	                        {$$ = $2; $$->next = $3; printf("indexeds <- ,indexedelem indexeds :line  %d  \n", yylineno);}
         |         		                                {$$ = NULL; printf("indexeds <- empty :line  %d  \n", yylineno);}
         ;

indexedelem : LEFT_CURLY expr COLON expr RIGHT_CURLY            {$$ = $4; $$->index = $2; printf("indexedelem <- { expr:expr } :line  %d  \n", yylineno);}
            ;
